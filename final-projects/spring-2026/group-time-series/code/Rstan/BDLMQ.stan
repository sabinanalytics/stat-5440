// co2_model_2_quadratic.stan
// =============================================================
// Quadratic observation extension of co2_model_2.stan.
//
// Structure:
//   y[i, t] = level[t]
//           + site_intercept[i]
//           + site_slope[i] * x_time[t]
//           + site_quadratic[i] * square(x_time[t])
//           + seasonal[i, t]
//           + noise_i
//
// The shared latent local-linear-trend state evolution is unchanged.
// The only extension is a demeaned hierarchical site-specific quadratic
// time term in the observation equation.
// =============================================================
...
parameters {
  ...
  vector[S] site_quadratic_raw;
  real<lower=0> sigma_site_quadratic;
...
transformed parameters {
  ...
  site_quadratic =
    sigma_site_quadratic * (site_quadratic_raw - mean(site_quadratic_raw));
...
model {
  ...
  site_quadratic_raw ~ std_normal();
  sigma_site_quadratic ~ exponential(2);
  ...
  for (n in 1:N) {
    ...
    real mu_n =
      ...
      site_quadratic[site[n]] * x_time_sq[t_id[n]] +
      seasonal_part;
...
generated quantities {
  ...
  for (n in 1:N_test) {
    ...
    mu_test[n] =
      ...
      site_quadratic[site_test[n]] * x_time_sq[t_test[n]] +
      seasonal_part;
